...
import {INameWrapper, CANNOT_UNWRAP, CANNOT_BURN_FUSES, CANNOT_TRANSFER, CANNOT_SET_RESOLVER, CANNOT_SET_TTL, CANNOT_CREATE_SUBDOMAIN, CANNOT_APPROVE, PARENT_CANNOT_CONTROL, CAN_DO_EVERYTHING, IS_DOT_MON, CAN_EXTEND_EXPIRY, PARENT_CONTROLLED_FUSES, USER_SETTABLE_FUSES} from "./INameWrapper.sol";
import {INameWrapperUpgrade} from "./INameWrapperUpgrade.sol";
import {IMetadataService} from "./IMetadataService.sol";
import {ENS} from "../registry/ENS.sol";
import {IReverseRegistrar} from "../reverseRegistrar/IReverseRegistrar.sol";
import {ReverseClaimer} from "../reverseRegistrar/ReverseClaimer.sol";
import {IBaseRegistrar} from "../monregistrar/IBaseRegistrar.sol";
    ...
    ReverseClaimer
{
    ...
    bytes32 private constant MON_LABELHASH =
    ...
    constructor(
        ...
        /* Burn PARENT_CANNOT_CONTROL and CANNOT_UNWRAP fuses for ROOT_NODE and MON_NODE and set expiry to max */
     ...
     * @param id Label as a string of the .mon domain to wrap
    ...
    function canModifyName(
        ...
        return
            ...
            !_isMON2LDInGracePeriod(fuses, expiry);
    }
    ...
    /**
     * @notice Wraps a .mon domain, creating a new token and sending the original ERC721 token to this contract
     * @dev Can be called by the owner of the name on the .mon registrar or an authorised caller on the registrar
     * @param label Label as a string of the .mon domain to wrap
    ...
    function wrapMON2LD(
        ...
        if (
            ...
            revert Unauthorised(
                _makeNode(MON_NODE, bytes32(tokenId)),
                msg.sender
            );
        }
    ...
    /**
     * @dev Registers a new .mon second-level domain and wraps it.
     *      Only callable by authorised controllers.
     * @param label The label to register (Eg, 'foo' for 'foo.mon').
     * @param wrappedOwner The owner of the wrapped name.
     * @param duration The duration, in seconds, to register the name for.
     * @param resolver The resolver address to set on the ENS registry (optional).
     * @param ownerControlledFuses Initial owner-controlled fuses to set
     * @return registrarExpiry The expiry date of the new name on the .mon registrar, in seconds since the Unix epoch.
     */
    ...
    /**
     * @notice Renews a .mon second-level domain.
     * @dev Only callable by authorised controllers.
     * @param tokenId The hash of the label to register (eg, `keccak256('foo')`, for 'foo.mon').
     * @param duration The number of seconds to renew the name for.
     * @return expires The expiry date of the name on the .mon registrar, in seconds since the Unix epoch.
     */
    ...
    function renew(
        ...
        bytes32 node = _makeNode(MON_NODE, bytes32(tokenId));
    ...
    /**
     * @notice Wraps a non .mon domain, of any kind. Could be a DNSSEC name vitalik.xyz or a subdomain
    ...
    /**
     * @notice Unwraps a .mon domain. e.g. vitalik.mon
     * @dev Can be called by the owner in the wrapper or an authorised caller in the wrapper
     * @param labelhash Labelhash of the .mon domain
     * @param registrant Sets the owner in the .mon registrar to this address
    ...
    function unwrapMON2LD(
        ...
        address registrant,
        address controller
    ) public onlyTokenOwner(_makeNode(MON_NODE, labelhash)) {
        if (registrant == address(this)) {
            revert IncorrectTargetOwner(registrant);
        }
        _unwrap(_makeNode(MON_NODE, labelhash), controller);
    ...
    /**
     * @notice Unwraps a non .mon domain, of any kind. Could be a DNSSEC name vitalik.xyz or a subdomain
     * @dev Can be called by the owner in the wrapper or an authorised caller in the wrapper
    ...
    /**
     * @notice Upgrades a domain of any kind. Could be a .mon name vitalik.mon, a DNSSEC name vitalik.xyz, or a subdomain
        ...
        )
    {
        ...
        if (owner == address(0)) {
            ...
            if (fuses & IS_DOT_MON == IS_DOT_MON) {
    ...
    function onERC721Received(
        ...
        //check if it's the mon registrar ERC721
        ...
        _wrapMON2LD(label, owner, ownerControlledFuses, expiry, resolver);
    ...
    function _beforeTransfer(
        ...
        // For this check, treat .mon 2LDs as expiring at the start of the grace period.
        if (fuses & IS_DOT_MON == IS_DOT_MON) {
            expiry -= GRACE_PERIOD;
        }
    ...
    function _normaliseExpiry(
        ...
        // .mon names will check registrar, non .mon check parent
        if (expiry > maxExpiry) {
            expiry = maxExpiry;
        }
    ...
    function _wrapMON2LD(
        ...
        bytes32 labelhash = keccak256(bytes(label));
        bytes32 node = _makeNode(MON_NODE, labelhash);
        // hardcode dns-encoded mon string for gas savings
        bytes memory name = _addLabel(label, "\x03mon\x00");
        names[node] = name;
        ...
        _wrap(
            ...
            name,
            wrappedOwner,
            fuses | PARENT_CANNOT_CONTROL | IS_DOT_MON,
            expiry
        );
    ...
    function _isMON2LDInGracePeriod(
        uint32 fuses,
        uint64 expiry
